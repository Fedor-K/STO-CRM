...
export default function UsersPage() {
  ...
  const { data, isLoading } = useQuery<PaginatedResponse>({
    ...
    queryFn: () =>
      apiFetch(`/users?page=${page}&limit=20&sort=createdAt&order=desc${roleFilter ? `&role=${roleFilter}` : ''}`),
  });
  ...
  return (
    <div>
      ...
      {isLoading ? (
        ...
        <div className="mt-8 text-center text-gray-500">Пользователей не найдено</div>
      ) : (
        <>
